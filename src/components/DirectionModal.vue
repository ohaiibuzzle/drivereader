...
const emit = defineEmits<{ select: [direction: 'ltr' | 'rtl'] }>()
</script>
      ...
      <div v-if="show" class="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm">
        <div class="bg-slate-800 rounded-2xl shadow-2xl max-w-sm w-full p-6 flex flex-col gap-5">
          <div class="text-center">
            <h2 class="text-white font-semibold text-lg">Reading direction</h2>
            <p class="text-slate-400 text-sm mt-1">How should pages turn?</p>
          </div>
          ...
          <div class="flex gap-3">
            ...
            <button
              class="flex-1 flex flex-col items-center gap-2 py-4 rounded-xl bg-slate-700 hover:bg-indigo-600 text-slate-300 hover:text-white transition-colors"
              @click="emit('select', 'rtl')"
            >
              <span class="text-2xl">←</span>
              <span class="text-sm font-medium">Right to Left</span>
              <span class="text-xs text-slate-400">Manga</span>
            </button>
          </div>
        </div>
      </div>
