...
<template>
  <header class="flex flex-col bg-slate-800 border-b border-slate-700 shrink-0">
    <!-- top bar -->
    <div class="flex items-center justify-between px-4 py-3 gap-3">
      <button class="flex items-center gap-2 hover:opacity-80 transition-opacity" @click="router.push({ name: 'home' })">
        <svg class="w-5 h-5 text-indigo-400 shrink-0" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5">
          <path stroke-linecap="round" stroke-linejoin="round" d="M12 6.042A8.967 8.967 0 006 3.75c-1.052 0-2.062.18-3 .512v14.25A8.987 8.987 0 016 18c2.305 0 4.408.867 6 2.292m0-14.25a8.966 8.966 0 016-2.292c1.052 0 2.062.18 3 .512v14.25A8.987 8.987 0 0018 18a8.967 8.967 0 00-6 2.292m0-14.25v14.25" />
        </svg>
        <span class="text-white font-semibold text-sm">DriveReader</span>
      </button>

      <div class="flex items-center gap-3">
        <!-- only shown when a folder is open -->
        <button
          v-if="breadcrumbs.length > 0"
          class="flex items-center gap-1.5 text-indigo-400 hover:text-indigo-300 text-sm font-medium transition-colors"
    ...
    <nav v-if="breadcrumbs.length > 0" class="flex items-center gap-1 px-4 pb-2 text-sm overflow-x-auto">
      <template v-for="(crumb, i) in breadcrumbs" :key="crumb.id">
        <span v-if="i > 0" class="text-slate-600 shrink-0">/</span>
        <button
